...
To do that, every `Expression` has a special node called the `head`, which is
the target node (variable) of the represented query. All operations over
`Expression` instances work on the `head` node, leaving the rest of the
nodes intact.

So `Expression` graphs are not built by explicitly adding nodes and edges like
any other normal graph. Instead they are built by a combination of the
following basic operations:

    - `__init__`: When a `Expression` is instantiated a single solitary node is
    ...
    - `add_data`: Adds a relation into some constant data from the `head` node
                  of the `Expression`. Used to represent stuff like
...
class Expression(object):
    ...
    def merge(self, other):
        ...
        node and every edge intact except for the `head`s. The `head`s
        are merged into a single node that is the new `head` and shares
        all the edges of the previous heads.
        """
    ...
    def decapitate(self, relation, reverse=False):
        ...
        If `reverse` is `True` then the `relation` links the old head to the
        new head instead of the opposite (some relations are not commutative).
        """
        oldhead = self.head
        self.head = self._add_node()
        if reverse:
